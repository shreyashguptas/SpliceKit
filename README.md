...
- **No telemetry.** This fork removed the Sentry crash reporter, the Sparkle update feed and the log-upload button that upstream bundles; nothing is sent anywhere unless you start it yourself (URL import, transcriber downloads, the install prerequisites). The full list of what touches the network, and when, is in [docs/THIRD_PARTY_DEPENDENCIES.md](docs/THIRD_PARTY_DEPENDENCIES.md). The runtime writes its log to `~/Library/Logs/SpliceKit/splicekit.log`, which is what to share in the [Discord](https://discord.com/invite/HD3FPc4Azu) or on [GitHub Issues](https://github.com/elliotttate/SpliceKit/issues) when something goes wrong.
