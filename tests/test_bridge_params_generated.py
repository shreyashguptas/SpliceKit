#!/usr/bin/env python3
"""Sources/Bridge/SpliceKitBridgeParams.m must match what scripts/gen_bridge_params.py generates."""
...
class BridgeParamsGeneratedTests(unittest.TestCase):
    def test_generated_params_table_is_current(self):
        proc = subprocess.run(
            [sys.executable, str(REPO_ROOT / "scripts" / "gen_bridge_params.py"), "--check"],
            capture_output=True, text=True)
        self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)

    def test_fcpxml_import_lists_path_and_async(self):
        text = (REPO_ROOT / "Sources" / "Bridge" / "SpliceKitBridgeParams.m").read_text(encoding="utf-8")
